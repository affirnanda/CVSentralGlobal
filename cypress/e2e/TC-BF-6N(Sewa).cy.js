describe("TC-BF-6N(Sewa)", () => {
    ...
    it("Admin menambah Harga produk melebihi max integer", () => {
        ...
        cy.get('textarea[name="description"]').type("Deskripsi produk");

        cy.get('input[name="price"]').type("2147483");

        cy.get('input[name="rental_price"]').type("2147483648");

        cy.get('input[name="stock"]').clear().type("11111");
        ...
        cy.contains("Nominal harga sewa terlalu besar").should("exist");
    });
});
